...
public final class CivilizationCommand {
    ...
    public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
        dispatcher.register(Commands.literal("civ")
                .executes(CivilizationCommand::help)
                .requires(source -> CivilizationConfig.debugCommandsEnabled()
                ...
                .then(Commands.literal("info")
                        .executes(CivilizationCommand::info))

                .then(Commands.literal("overview")
                        .executes(CivilizationCommand::overview))
                ...
                .then(Commands.literal("map")
                        .executes(CivilizationCommand::map))
                ...
                .then(Commands.literal("list")
                        .executes(CivilizationCommand::list))

                .then(Commands.literal("wall")
                        .executes(CivilizationCommand::wall)
                        .then(Commands.literal("complete")
                                .executes(CivilizationCommand::wallComplete))
                        .then(Commands.literal("map")
                                .executes(CivilizationCommand::wallMap)))

                .then(Commands.literal("buildstop")
                        .executes(CivilizationCommand::buildStop))
                ...
                .then(Commands.literal("plan")
                        .executes(CivilizationCommand::plan))

                .then(Commands.literal("oracle")
                        .executes(CivilizationCommand::oracle))
        ...
        for (Kingdom kingdom : world.kingdoms()) {
            for (Settlement settlement : kingdom.settlements()) {
                ...
                for (TownAuditor.Fault fault : faults) {
                    ...
                    CivilizationMod.LOGGER.info("AUDIT {} {}", settlement.name(), fault.describe());
                }
                total += faults.size();
            }
        }
    ...
    private static int sites(CommandContext<CommandSourceStack> ctx, int reach) {
        ...
        SettlementSites.Grid grid =
                com.civilization.neoforge.world.WorldgenSettlements.gridFor(level);
    ...
    private static int found(CommandContext<CommandSourceStack> ctx, String name) {
        ...
        SimPos chosen = Founding.bestSiteNear(center, Founding.SITING_REACH,
                CivilizationMod.simulationFor(level).bridge());
        if (!chosen.equals(wanted)) {
            CivilizationMod.LOGGER.info("FOUND moved {} from {} to {} for better ground",
                    name, wanted, chosen);
        }
     ...
     */
    private static final String SEEDED_CULTURE = "civilization:human/norman";
    ...
    private static int seed(CommandContext<CommandSourceStack> ctx, String stageName,
        ...
        if (!chosen.equals(wanted)) {
            CivilizationMod.LOGGER.info("SEED moved from {} to {} for better ground",
                    wanted, chosen);
        }
        ...
        CivilizationMod.LOGGER.info("SEED {} stage={} at {} buildings={} pop={}",
    ...
    private static int info(CommandContext<CommandSourceStack> ctx) {
        ...
        PersonEntityManager digger = CivilizationMod.managerFor(source.getLevel());
        if (world == null) {
            source.sendFailure(Component.literal("No simulation for this dimension."));
            return 0;
        }
     ...
     * {@code civilization:default} and nothing ever picked another. So the layouts,
    ...
    private static int culture(CommandContext<CommandSourceStack> ctx, String id) {
        ...
        CivilizationMod.LOGGER.info("CULTURE {} -> {} ({})",
    ...
    private static int wallComplete(CommandContext<CommandSourceStack> ctx) {
        ...
        com.civilization.sim.settlement.Perimeter ring = settlement.perimeter();
        if (ring == null) {
            source.sendFailure(Component.literal(
        ...
        CivilizationMod.LOGGER.info("WALLCOMPLETE {} {} -> {} gates={}",
    ...
    private static int wallMap(CommandContext<CommandSourceStack> ctx) {
        ...
        java.util.List<com.civilization.sim.geom.SimPos> line = ring.ringPositions();
        int west = Integer.MAX_VALUE;
        int east = Integer.MIN_VALUE;
        int north = Integer.MAX_VALUE;
        int south = Integer.MIN_VALUE;
        for (com.civilization.sim.geom.SimPos at : line) {
        ...
        for (com.civilization.sim.settlement.Building building : settlement.buildings()) {
            com.civilization.sim.geom.SimPos at = building.origin();
        ...
        CivilizationMod.LOGGER.info("WALLMAP {} posts={} vertices={} box={}x{} at {},{}",
                settlement.name(), line.size(), ring.vertices().size(),
                width, depth, west, north);
        for (char[] row : grid) {
            CivilizationMod.LOGGER.info("WALLMAP |{}|", new String(row));
        }
        source.sendSuccess(() -> Component.literal(
    ...
    private static int oracle(CommandContext<CommandSourceStack> ctx) {
        CommandSourceStack source = ctx.getSource();
        ServerLevel level = source.getLevel();
        com.civilization.neoforge.world.TerrainOracle oracle =
                ((com.civilization.neoforge.bridge.NeoForgeWorldBridge)
                        CivilizationMod.simulationFor(level).bridge()).oracle();
        ...
        for (int dz = -96; dz <= 96; dz += 4) {
            for (int dx = -96; dx <= 96; dx += 4) {
                ...
                if (com.civilization.neoforge.world.WallClearing.isGrowth(under)) {
                    continue;
                }
                boolean reallyWet =
                        !level.getFluidState(
        ...
        source.sendSuccess(() -> Component.literal(report), false);
        CivilizationMod.LOGGER.info("ORACLE compared={} exactPct={} meanErr={} worst={} "
    ...
    private static int plan(CommandContext<CommandSourceStack> ctx) {
        ...
        CivilizationMod.LOGGER.info("PLAN TOWN {} {} {} {} {} {} {} {}",
        ...
        for (com.civilization.sim.settlement.Building b : settlement.buildings()) {
            SimPos at = b.origin();
            com.civilization.sim.settlement.Footprint print = b.footprint();
            ...
            // whichever is wrong.
            CivilizationMod.LOGGER.info("PLAN B {} {} {} {} {} {} {} {} {} {}",
                    b.blueprintId(), at.x(), at.y(), at.z(),
                    com.civilization.sim.settlement.BuildPlanner.plotSpanOf(
                            b.blueprintId(), settlement.catalog()),
                    com.civilization.sim.settlement.BuildPlanner.facingToward(at, center),
                    b.role(),
        ...
        java.util.List<com.civilization.sim.settlement.PathNetwork.Segment> runs =
                settlement.paths().segments();
        for (int i = 0; i < runs.size(); i++) {
            com.civilization.sim.settlement.PathNetwork.Segment seg = runs.get(i);
            ...
            CivilizationMod.LOGGER.info("PLAN R {} {} {} {} {} {}",
        ...
        com.civilization.neoforge.world.TerrainOracle roadGround =
                ((com.civilization.neoforge.bridge.NeoForgeWorldBridge)
                        CivilizationMod.simulationFor(level).bridge()).oracle();
        for (int i = 0; i < runs.size(); i++) {
            ...
            CivilizationMod.LOGGER.info("PLAN RS {} {}", i, profile);
        }
        ...
        if (ring != null) {
            ...
            CivilizationMod.LOGGER.info("PLAN W {} {} {}",
            ...
            CivilizationMod.LOGGER.info("PLAN G {}", gates.toString().trim());
            ...
            for (int i = 0; i < posts.size(); i++) {
                ...
                if (row.length() > 3000) {
                    CivilizationMod.LOGGER.info("PLAN P {}", row.toString().trim());
                    row.setLength(0);
                }
            }
            if (row.length() > 0) {
        ...
        for (int dz = -half; dz <= half; dz += PLAN_STEP) {
            ...
            CivilizationMod.LOGGER.info("PLAN H {} {} {}", dz, heights, wet);
        }
    ...
    private static int terrain(CommandContext<CommandSourceStack> ctx, int radius) {
        ...
        for (int iz = 0; iz < cells; iz++) {
            ...
            for (int ix = 0; ix < cells; ix++) {
                ...
                int height = com.civilization.neoforge.world.BlueprintPlacer
        ...
        source.sendSuccess(() -> Component.literal(
                "  wrote " + cells + "x" + cells + " cells to " + file.getFileName()
                        + " in " + took + "ms"), true);
        CivilizationMod.LOGGER.info("TERRAIN wrote {} cells to {} in {}ms",
    ...
    private static int wall(CommandContext<CommandSourceStack> ctx) {
        ...
        java.util.List<com.civilization.sim.geom.SimPos> positions = ring.ringPositions();
        int laid = Math.min(ring.laid(), positions.size());
        for (int i = 0; i < laid; i++) {
            com.civilization.sim.geom.SimPos at = positions.get(i);
            ...
            net.minecraft.core.BlockPos ground =
                    com.civilization.neoforge.world.PerimeterLayer.footingFor(level, at);
            if (ground == null) {
                unfooted++;   // water, or no ground the layer would accept
                continue;
            }
            net.minecraft.core.BlockPos growth =
                    com.civilization.neoforge.world.WallClearing.inTheWay(level, ground);
            if (growth != null) {
                blocked++;
            ...
            if (com.civilization.neoforge.world.PerimeterLayer.postStands(level, ground)) {
                standing++;
            } else if (com.civilization.neoforge.world.PerimeterLayer
        ...
        int out_ = 0;
        int shutOut = 0;
        for (com.civilization.sim.person.Person person : settlement.residents()) {
            if (!person.isEmbodied()) {
                continue;
            }
            com.civilization.sim.geom.SimPos at = person.position();
            boolean within = com.civilization.sim.geom.Hull.contains(ring.vertices(), at);
            if (within) {
                in++;
                continue;
            }
            out_++;
            for (com.civilization.sim.person.Household household : settlement.households()) {
                if (!household.isHoused() || !household.members().contains(person.id())) {
                    continue;
                }
                if (com.civilization.sim.geom.Hull.contains(
    ...
    private static int stores(CommandContext<CommandSourceStack> ctx) {
        ...
        Building market = settlement.buildingWithRole(
                com.civilization.sim.settlement.BuildingRole.MARKET);
        boolean trader = com.civilization.sim.economy.Market.hasTrader(settlement);
        out.append("\n  market: ");
        if (market == null) {
            ...
            for (com.civilization.sim.economy.Market.Deal deal
                    : com.civilization.sim.economy.Market.offers(settlement)) {
                out.append("\n    ").append(deal.townBuys() ? "buys  " : "sells ")
                        ...
                        .append(com.civilization.sim.economy.Market.LOT);
            }
            CivilizationMod.LOGGER.info("MARKET {} at {} {} {} trader={} offers={}",
                    settlement.name(), market.origin().x(), market.origin().y(),
                    market.origin().z(), trader,
                    com.civilization.sim.economy.Market.offers(settlement).size());
        }
        ...
        CivilizationMod.LOGGER.info(
                ...
                settlement.stores().get(com.civilization.sim.settlement.TownStores.STONE),
                settlement.foodStock(),
                settlement.stores().get(com.civilization.sim.settlement.TownStores.IRON),
    ...
    private static int map(CommandContext<CommandSourceStack> ctx) {
        ...
        if (!TownMaps.open(player, nearest)) {
            ...
            source.sendFailure(Component.literal(
                    "This client cannot draw the map — Civilization is not installed on it."));
            return 0;
        }
        return 1;
    }
