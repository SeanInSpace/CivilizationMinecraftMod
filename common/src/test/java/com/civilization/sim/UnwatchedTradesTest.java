...
class UnwatchedTradesTest {
    ...
    void aFoundingPartyOnRealGroundBuildsWithoutBeingWatched() {
        ...
        assertTrue(camp.countBuildings("civilization:cottage")
                        + camp.countBuildings("civilization:house") >= 1,
                "a roof was raised for somebody who was not on the charter." + log);
        assertTrue(camp.countBuildings("civilization:farm") >= 2,
                "and a second field dug, which is the thing a town with no timber"
                        + " could never get to." + log);
        assertTrue(camp.tallies().get(com.civilization.sim.settlement.Tallies.TREES_FELLED) > 0,
                "off real trees." + log);
    ...
    private static Settlement seededVillage() {
        ...
        String[] plan = {"civilization:hearth", "civilization:granary", "civilization:farm",
                "civilization:farm", "civilization:lumber_camp", "civilization:mine",
                "civilization:house", "civilization:house"};
