...
public final class Founding {
     ...
     *
     * <p>The same storey {@code Settlement.expectedFootprint} assumes for a
     ...
     *
     * <p>The whole cumulative programme is fifteen buildings; this is room for
     ...
     * programme entries this catalogue has never heard of. The professions are
     * whatever {@link StagePlanner#crystallize} and {@link JobPlanner} name. The
     * plots are the town plan's own, taken nearest-centre first, so the town
     * fills outward from the post it staked on the day it arrived.
     *
     * <p><strong>The hall is not in the middle</strong>, and that is the
     * programme's doing rather than an oversight. It is the last entry of the
     * last stage — the capstone the whole staging design exists to hold back —
     * so the plots nearer the centre are long since taken by the camp post, the
     ...
     * perimeter, and {@code Settlement.chooseSite} rescans from the centre for
     * civic buildings once there is a ring to stay inside. A seeded town has no
     * ring yet, so it has no such gap to fill.
     *
     * <p><strong>The programmes are cumulative.</strong> A village did not skip
     * its bunkhouse on the way past, so a seeded one has not either. What it
     * does <em>not</em> have is anything the catalogue scan would have added
     * along the way — a second granary, the houses a growing town raises — and
     * that is deliberate: the programme is a definition, and "whatever the scan
     ...
     * whole of a stage's programme is, by {@link StagePlanner#readyToAdvance}'s
     ...
     * its programme, and the bar this method is held to is being
     ...
     *
     * <p>The buildings are recorded <em>unmaterialised</em>. A seeded town is
     ...
     * centre's height as an estimate and are marked unsurveyed, so the placement
     ...
     *                  for as many as the stage's programme has beds for
     */
    public static Settlement seeded(SimPos site, String name, SettlementStage stage,
                                    List<BuildingType> catalogue, String cultureId,
                                    int residents) {
    ...
    /**
     * Every stage's programme up to this one, standing, in the order they were
     * wanted.
     *
     * <p>The stage is set and re-asked rather than the programmes being read out
     * of {@link StagePlanner}, because the programmes are private and should
    ...
    private static void raiseTheProgrammes(Settlement town, SettlementStage upTo) {
        ...
        for (SettlementStage reached : SettlementStage.values()) {
            ...
            // Bounded by the plan rather than by the programme: a want that
            // could not be placed must end the loop, or a plan smaller than the
            // programme would spin here forever.
            while (taken < plan.size()) {
                ...
                if (taken >= plan.size()) {
                    // The plan ran out before the programme did. Nothing today
                    // reaches this — the ring layout spends nineteen of sixty-four
                    // plots on the whole fifteen-building programme — but the
                    // programme, the plot spans and the per-culture layouts all
                    ...
                    town.logEvent(BEFORE_THE_FIRST_STEP, "Seeded short: the "
                            + reached.pretty() + " programme wanted a "
    ...
    /** One building of the programme, as the unwatched clock would have left it. */
    ...
    /** Far enough out to hold everything the programmes put on the ground. */
     ...
     * house cannot bear and cannot split — so the beds the programme raised are
     ...
     * and cruder — it groups by the <em>largest</em> capacity in the catalogue
    ...
    private static void settlePeople(Settlement town, int wanted) {
        ...
        // catalogue, which is exactly what PopulationPlanner would do with them
    ...
    /**
     * The most beds any house in this catalogue has, which is how big a family
     * the planner gathers strangers into.
     *
     * <p>Mirrors {@code PopulationPlanner.largestHousingCapacity}, which is
     * private. One is the floor: a catalogue with no housing in it would
     ...
     * founding kit — {@code Settlement.loosePile} is initialised with it — so
     ...
     * materials figure in this codebase sized against a real programme (see
     ...
     * than left to the field initialiser, so what a seeded town holds is a
